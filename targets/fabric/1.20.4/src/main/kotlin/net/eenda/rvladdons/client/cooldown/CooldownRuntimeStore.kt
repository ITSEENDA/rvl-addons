...
internal class CooldownRuntimeStore(private val pending: CooldownPendingActionStore) {
    ...
    fun clear() { pending.clear(); icons.clear(); lastSkillKey = null; registry.clear() }
    fun removeProfile(skillKey: String) { registry.remove(skillKey); icons.remove(skillKey); pending.removeIf { it.source?.skillKey == skillKey }; if (lastSkillKey == skillKey) lastSkillKey = null }
    fun active(now: Long = System.currentTimeMillis()): List<CooldownEntry> { registry.active(now).filter { isPassive(it.trigger) || SkillProfileStore.find(it.skillKey)?.cooldownHintMs == null }.forEach { registry.remove(it.skillKey) }; return registry.active(now) }
    ...
    fun removeEstimated(skillKey: String) { val entry = registry.get(skillKey); if (entry?.confidence == CooldownConfidence.ESTIMATED) { registry.remove(skillKey); icons.remove(skillKey) } }
    fun startEstimated(action: PendingSkillAction) {
        ...
        val profile = SkillProfileStore.find(key) ?: SkillProfileStore.register(key, source.displayName, source.setId, action.trigger, source.fingerprint, source.cooldownHintMs, source.sourceType, source.skillType, source.setBonusLevel)
        ...
        if (existing != null && existing.endAt > action.at) { lastSkillKey = key; return }
        registry.upsert(CooldownEntry(key, profile.label, action.trigger, source, action.at, action.at + hint, hint, action.at, CooldownConfidence.ESTIMATED))
        lastSkillKey = key
        RvlAddonsTrace.log("cooldown", "estimated-start key=$key hintMs=$hint")
    }
    private fun isPassive(trigger: SkillTrigger): Boolean = trigger == SkillTrigger.PASSIVE_DAMAGED || trigger == SkillTrigger.PASSIVE
}
