    ...
    override fun onUpdate() {
        ...
        if (current.steps.peekFirst()?.role?.startsWith("move-") == true) {
            ...
            if (clearClicks > 0 && delay > 0L) {
                ...
                owner.notify(client, "RVL COMA: local clear burst sent ($clearClicks), waiting ${delay}ms")
                RvlAddonsTrace.log(
                    "coma",
                ...
                waitForMove = false
                if (clearClicks > 0) {
                    owner.notify(client, "RVL COMA: local clear burst sent ($clearClicks), moving immediately")
                    RvlAddonsTrace.log(
                        "coma",
                        "local-clear-burst-submitted syncId=${handler.syncId} clicks=$clearClicks"
                    )
