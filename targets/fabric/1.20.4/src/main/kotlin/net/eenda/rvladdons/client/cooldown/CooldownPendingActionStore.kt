...
internal class CooldownPendingActionStore {
    ...
    fun clear() { actions.clear(); icons.clear() }
    fun prune(now: Long) { actions.removeIf { now - it.at > 2_000L } }
    fun remove(action: PendingSkillAction) { actions.remove(action); icons.remove(action) }
    fun removeIf(predicate: (PendingSkillAction) -> Boolean) { actions.filter(predicate).forEach(::remove) }

    fun queue(action: PendingSkillAction, icon: ItemStack?, sourceStack: (PendingSkillAction) -> ItemStack?): List<PendingSkillAction> {
        ...
        if (action.hand == SkillHand.OFF && sameGesture.any { it.hand == SkillHand.MAIN && it.source?.skillKey != null }) return removed
        if (action.hand == SkillHand.MAIN && action.source?.skillKey != null) {
            sameGesture.filter { it.hand == SkillHand.OFF }.forEach { old -> remove(old); removed += old }
        }
    ...
    fun cancelGesture(trigger: SkillTrigger, now: Long): List<PendingSkillAction> {
        val removed = actions.filter { it.trigger == trigger && now - it.at in 0..CooldownActionTracker.HAND_DUPLICATE_WINDOW_MS }
        removed.forEach(::remove)
        return removed
    }

    fun select(now: Long, remainingMs: Long): PendingSkillAction? = actions.filter { now - it.at in 0..900L }
        .maxByOrNull { action ->
            var score = if (action.source?.skillKey != null) 100.0 else 0.0
            if (action.hand == SkillHand.MAIN) score += 20.0
            action.source?.cooldownHintMs?.let { hint -> score -= minOf(80.0, kotlin.math.abs(hint - remainingMs).toDouble() / 1000.0) }
